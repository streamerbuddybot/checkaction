 ...
 * git: https://github.com/thesayyn/protoc-gen-ts */
 ...
 export class spotifyQueueResponse extends pb_1.Message {
     ...
     get songDetails() {
         return pb_1.Message.getRepeatedWrapperField(this, spotifySongDetails, 3) as spotifySongDetails[];
     }
     set songDetails(value: spotifySongDetails[]) {
     ...
     static fromObject(data: {
         ...
         if (data.songDetails != null) {
             message.songDetails = data.songDetails.map(item => spotifySongDetails.fromObject(item));
         }
         return message;
     }
     toObject() {
         ...
         if (this.songDetails != null) {
             data.songDetails = this.songDetails.map((item: spotifySongDetails) => item.toObject());
         }
         return data;
     }
     serialize(): Uint8Array;
     ...
     serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
         ...
         if (this.statusMessage.length)
             writer.writeString(2, this.statusMessage);
         if (this.songDetails.length)
             writer.writeRepeatedMessage(3, this.songDetails, (item: spotifySongDetails) => item.serialize(writer));
         if (!w)
             return writer.getResultBuffer();
     }
         ...
         while (reader.nextField()) {
             ...
             switch (reader.getFieldNumber()) {
                 ...
                 case 2:
                     ...
                     break;
                 case 3:
                     reader.readMessage(message.songDetails, () => pb_1.Message.addToRepeatedWrapperField(message, 3, spotifySongDetails.deserialize(reader), spotifySongDetails));
                     break;
                 default: reader.skipField();
             }
         }
